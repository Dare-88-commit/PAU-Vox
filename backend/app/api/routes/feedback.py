...
from app.services.profanity import contains_profanity
from app.services.similarity import detect_similarity_group
from app.ai.profanity import check_profanity
...
def _serialize_feedback(feedback: Feedback, viewer_role: UserRole) -> FeedbackOut:
    ...
    return FeedbackOut(
        ...
        resolution_summary=feedback.resolution_summary,
        similarity_group=feedback.similarity_group,
        attachments=[f"/api/v1/feedback/{feedback.id}/attachments/{att.id}" for att in feedback.attachments],
        created_at=feedback.created_at,
        updated_at=feedback.updated_at,
        notes=feedback.notes if include_internal else [],
        status_history=feedback.status_history if include_internal else [],
...
def _record_status_change(db: Session, feedback: Feedback, status_value: FeedbackStatus, by_user_id: str, note: Optional[str]):
    feedback.status = status_value
    feedback.updated_at = datetime.now(timezone.utc)
    db.add(StatusHistory(feedback_id=feedback.id, status=status_value, updated_by_id=by_user_id, note=note))
...
<<<<<<< HEAD
def _notify(db: Session, user_id: str, title: str, message: str, feedback_id: Optional[str] = None, level: NotificationType = NotificationType.info):
=======
def _notify(
    db: Session,
    user_id: str,
...
def _can_modify_status(current_user: User, feedback: Feedback) -> bool:
    role = current_user.role
<<<<<<< HEAD
    if role in {UserRole.academic_staff, UserRole.department_head}:
...
@router.post("/", response_model=FeedbackOut, status_code=status.HTTP_201_CREATED)
def create_feedback(payload: FeedbackCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> FeedbackOut:
    if current_user.role != UserRole.student:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only students can submit feedback")
    department = normalize_department(payload.department)
    if payload.type == FeedbackType.academic and not department:
...
>>>>>>> 729e00501dbbf5426a5acc6f287fc9fd111f666a
    ...
    if check_profanity(payload.subject) or check_profanity(payload.description):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Feedback contains blocked language")

    # Urgency prediction
    try:
        priority = FeedbackPriority(urgency_model.predict(payload.description))
    except Exception:
        priority = FeedbackPriority.medium

    similarity_group = detect_similarity_group(db, payload.type, payload.category, payload.subject, payload.description)
    ...
    db.add(feedback)
    db.flush()
    db.add(StatusHistory(feedback_id=feedback.id, status=FeedbackStatus.pending, updated_by_id=current_user.id, note="Initial submission"))
...
@router.get("/", response_model=FeedbackListResponse)
def get_all_feedback(db: Session = Depends(get_db), current_user: User = Depends(get_current_user), priority: Optional[FeedbackPriority] = Query(None), status: Optional[FeedbackStatus] = Query(None)) -> FeedbackListResponse:
    query = db.query(Feedback).options(
        joinedload(Feedback.student),
        joinedload(Feedback.notes),
        joinedload(Feedback.status_history),
...
<<<<<<< HEAD
    feedbacks = query.all()
    return FeedbackListResponse(feedbacks=[_serialize_feedback(fb, current_user.role) for fb in feedbacks])
=======
    if current_user.role == UserRole.student:
...
@router.patch("/{feedback_id}", response_model=FeedbackOut)
def update_feedback_by_student(feedback_id: str, payload: FeedbackUpdateByStudent, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> FeedbackOut:
    feedback = db.query(Feedback).options(joinedload(Feedback.student), joinedload(Feedback.attachments)).filter(Feedback.id == feedback_id).first()
    if not feedback:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Feedback not found")
    if feedback.student_id != current_user.id or current_user.role != UserRole.student:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the submitting student can edit this feedback")
    ...
    feedback.priority = detect_priority(feedback.description)
    feedback.similarity_group = detect_similarity_group(db, feedback.type, feedback.category, feedback.subject, feedback.description)
    feedback.updated_at = datetime.now(timezone.utc)
    db.commit()
    db.refresh(feedback)
